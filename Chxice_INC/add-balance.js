...
const mention = mentions.users.first()
                if(!mention){
                    message.reply('Please provide a user').then(msg=>msg.delete({timeout:'5000'}))
                }
const coins = parts[2]
if(isNaN(coins)) return message.reply('Please provide a number').then(msg=>msg.delete({timeout:'5000'})) 
const guildId = message.guild.id 
const userId = mention.id

const newCoins = await economy.addCoins(guildId, userId, coins)

message.reply(`You gave ${mention} ${coins} coins`).then(msg=>msg.delete({timeout:'5000'})) 
               }}}})
}