...
distube
    ...
    setTimeout(() => message.channel.bulkDelete(4), 2000)})
...
client.on('message' , async (message) => {if(!message.member.bot && message.guild){
    ...
    const text = parts[1]
  if (!message.member.voice.channel) return message.channel.send('Du bist in keinem Sprachkanal').then(msg=>msg.delete({timeout:'5000'}))
...
message.delete()
  ...
  if (!message.member.voice.channel) return message.channel.send('Du bist in keinem Sprachkanal').then(msg=>msg.delete({timeout:'5000'}))
  if(bot.voice.channel !== message.member.voice.channel) return message.channel.send('Fehler').then(msg=>msg.delete({timeout:'5000'}))
distube.stop(message)
message.delete()
message.reply('Du hast die Musik gestoppt').then(msg=>msg.delete({timeout:'5000'}))
