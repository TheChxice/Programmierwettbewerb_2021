module.exports = client => {
  const { Leaver } = require('canvacord')
  const fs = require('fs')
  const canvacord =require('canvacord')
  
    const Discord = require('discord.js')

    client.on('guildMemberRemove' , async (member) => {
      const kanal = JSON.parse(fs.readFileSync('channelbye.json', 'utf8'))
  let chanel_ = kanal.channel1[kanal.channel1.length -1]
const channelID3 = chanel_.channel4
        console.log(member)
        const goodbye = new Leaver()
        .setUsername(member.user.username)
        .setDiscriminator(member.user.discriminator)
        .setMemberCount(' ')
        .setGuildName(member.guild.name)
        .setColor("border", "#12c4b8")
        .setColor("username-box", "#12c4b8")
        .setColor("discriminator-box", "#12c4b8")
        .setColor("message-box", "#12c4b8")
        .setColor("title", "#12c4b8")
        .setColor("avatar", "#12c4b8")
        .setText("member-count", " ")
        .setText("title", "Tschüss")
        .setText("message", "Schönen Tag noch")
        .setAvatar(member.user.displayAvatarURL({format: "png"}))
        let attatchment = new Discord.MessageAttachment(await goodbye.build(), "bye.png")
      
    
      const channel = member.guild.channels.cache.get(channelID3)
      channel.send(attatchment)
      })
      
}